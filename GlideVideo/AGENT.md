...
## Overview
GlideVideo adds touch gestures and a controller overlay to HTML5 video players on mobile browsers. TypeScript + Vite, shipped as **two builds off one `src/`**:

| Target | Entry | Config | Output |
|---|---|---|---|
| Userscript (Greasemonkey/Tampermonkey) | `src/index.ts` | `vite.config.ts` (`vite-plugin-monkey`) | `dist/glidevideo.user.js` |
| Firefox extension (MV3) | `src/platform/extension-entry.ts` | `vite.extension.config.ts` | `dist-extension/` |

Nothing under `src/` except the two entry files knows which build it is in.

## Structure
```
src/
├── config.ts               # Central constants (offsets, speeds, timeouts, thresholds)
├── index.ts                # Userscript entry point
├── platform/
│   └── extension-entry.ts  # Firefox MV3 content-script entry: hydrates extension storage into GM_* shims, then boots Controller
...
└── video/
    ...
    └── VideoTransform.ts   # Aspect ratio/zoom styling, rate/skip/play-pause actions, overlay repositioning

extension/                  # Static MV3 assets copied verbatim into dist-extension/
├── manifest.json           # version + description overwritten from package.json at build
├── background.js           # Event page: toolbar button toggles the current site
└── icon.svg
```
...
- Key Commands:
  ...
  - `bun run build` - Build both targets
  - `bun run build:userscript` - Userscript only → `dist/glidevideo.user.js`
  - `bun run build:extension` - Firefox MV3 only → `dist-extension/`
...
- Site-level Enable/Disable: `disabled_<hostname>` in storage, read at boot by both entries. The userscript exposes it as a GM menu command; the extension exposes it as the toolbar button (`extension/background.js`), which needs only `activeTab` because `tab.url` is granted on click.
- Storage goes through private `storageGet`/`storageSet` helpers: cross-site GM storage (GM_getValue/GM_setValue) first, localStorage fallback for iframe storage restrictions. In the extension build those globals are shims over `browser.storage.local`, hydrated into a synchronous cache before `Controller` is constructed — StateStore reads settings synchronously in its constructor, so the async read has to finish first.
...
## Blunders
- **Invisible chrome still takes taps**: `attachUIToVideo()` sets `wrap.style.display = "block"` on every attach, but `wrap` stays at `opacity: 0` until `showUI()` runs — and `showUI()` is gated behind `INTERACTION_TIMEOUT` against `lastRealUserEvent`, which starts at 0. So from page load until the user first revealed the UI, the top bar was invisible with `pointer-events: auto` on every control: a tap top-left silently changed playback speed. Fixed once at the shared point with `.mvc-ui-wrap:not(.mvc-shown) * { pointer-events: none }`, toggled by `showUI`/`hideUI` — not per control, because everything inside `wrap` opts *into* pointer-events and so everything has to opt back out.
- **The lock shield cannot be a child of `wrap`**: `hideUI()` sets `wrap.style.display = "none"` after the fade, which took the shield with it — the screen lock silently stopped blocking gestures ~3.9s after being switched on while the button still read "locked". It now lives beside `wrap` in the fullscreen container, which makes it a tenth overlay that must appear in `VideoTransform.onFullScreenChange()`, `Controller.destroy()` *and* `isPointOnUI()`.
- **Attribute selectors outrank plain classes**: `:root[data-mvc-left-hand="true"] .mvc-volume-bar` is (0,3,0) and beats `.mvc-volume-bar.visible` at (0,2,0), so in left-hand mode the rails never animated in — they sat at their hidden transform forever. Any `:root[data-…]` themed rule that sets a property a state class also sets has to restate the state class.
- **`wrap` was never positioned by its own left/top**: both of its children are `position: fixed`, so ~140 lines of page-position maths (`_applyPagePosition`, `ensureUIInViewport`, `getViewportPageBounds`, `findScrollableParents` and the per-parent scroll listeners) moved nothing on screen. What actually tracks the video is the set of overlays that copy its bounding rect, via `ui.updateBrightnessOverlayPosition()`. Nested scroll containers are now covered by one capture-phase `scroll` listener on `window` — scroll events do not bubble, but they do capture.

(function() {
	...
	*/
	...
	//#endregion
	//#region src/gestures/DoubleTapDetector.ts
	...
	//#endregion
	//#region src/gestures/PressDetector.ts
	...
	//#endregion
	//#region src/gestures/SwipeDetector.ts
	...
	*/
	var THEMES = `
        ...
        :root[data-mvc-theme="contrast"] {
            ...
            --mvc-switch-off: rgba(255, 255, 255, 0.16);
            --mvc-thumb: #ffffff;
            --mvc-ease: linear;
            --mvc-dur: 0.1s;
            --mvc-glyph-shadow: none;
        ...
        :root[data-mvc-theme="frame"] {
            ...
            --mvc-switch-off: rgba(242, 240, 236, 0.28);
            --mvc-thumb: #ffffff;
            --mvc-ease: linear;
            --mvc-dur: 0.12s;
...
`;
	function injectStyles() {
		...
		style.textContent = `
${THEMES}
        /* Until the chrome is actually shown, nothing inside it may take a tap.
           wrap is set to display:block the moment it attaches to a video but
           stays at opacity 0 until showUI() runs — and an invisible button
           still receives pointerdown. Gate it here, once, rather than on each
           control: everything inside wrap opts into pointer-events, so
           everything inside wrap has to opt back out. */
        .mvc-ui-wrap:not(.mvc-shown) * {
        ...
        /* These beat .mvc-volume-bar.visible on specificity (0,3,0 vs 0,2,0),
           so the matching .visible rules have to be restated here or the
           rails never animate in — they just sit at their hidden transform. */
        ...
        :root[data-mvc-left-hand="true"] .mvc-volume-bar.visible,
        :root[data-mvc-left-hand="true"] .mvc-brightness-bar.visible {
        ...
        .mvc-settings-sheet {
            ...
            -webkit-backdrop-filter: var(--mvc-blur);
            box-shadow: var(--mvc-shadow);
            padding: 14px 16px max(20px, calc(env(safe-area-inset-bottom, 0px) + 16px));
            opacity: 0;
        ...
        /* Switch */
        /* The switch track is NOT --mvc-track: that token colours the scrubber
           floating on the picture, and a theme whose sheet is a different
           lightness than the video needs the two to disagree. */
        .mvc-switch {
            position: relative;
            width: 38px;
            height: 22px;
            flex-shrink: 0;
            background: var(--mvc-switch-off);
            border-radius: var(--mvc-r-pill);
        ...
        .mvc-toast {
            position: fixed;
            left: 50%;
            bottom: max(40px, calc(env(safe-area-inset-bottom, 0px) + 30px));
	...
	//#endregion
	//#region src/ui/components/ProgressBar.ts
	...
	*/
	...
	//#endregion
	//#region src/ui/components/Stepper.ts
	...
	//#endregion
	//#region src/ui/panels/SettingsSheet.ts
	...
	//#endregion
	//#region src/ui/panels/SpeedStepper.ts
	...
	//#endregion
	//#region src/video/VideoAdapter.ts
	...
	//#endregion
	//#region src/video/VideoTracker.ts
	/**
	...
	*/
	...
	var VideoTracker = class {
		...
		mutationObserver;
		resizeObserver;
		shadowObservers = /* @__PURE__ */ new Map();
		...
		*/
		/**
		* Tracks every video handed to watchVideo(), not just the ones that became
		* visible — destroy() used to walk `visibleVideos` and so left listeners on
		* every video that was watched but never intersected.
		*/
	...
	//#endregion
	//#region src/video/VideoTransform.ts
	...
	var VideoTransform = class {
		...
		setupObservers() {
			...
			this.videoMutationObserver = new MutationObserver(() => this.throttledReposition());
		}
		attachGlobalListeners() {
		...
		attachUIToVideo(video) {
			...
			const container = getFullscreenContainer();
			if (container && container.isConnected) container.appendChild(this.ui.wrap);
			...
			switch (event.type) {
				...
				case "play":
					if (this.store.activeVideo) {
						...
						const targetRate = this.store.settings.rememberPlayback ? this.store.settings.lastRate || this.store.settings.defaultSpeed || 1 : this.store.getVideoMetadata(this.store.activeVideo).lastRate || this.store.settings.defaultSpeed || 1;
						if (this.store.activeVideo.playbackRate !== targetRate && this.store.savedPlaybackRate === void 0) this._setRate(targetRate, false);
					}
	...
	//#endregion
	//#region src/gestures/GestureCoordinator.ts
	...
	//#endregion
	//#region src/core/StateStore.ts
	var StateStore = class {
		eventBus;
		activeVideo = null;
		visibleVideos = /* @__PURE__ */ new Map();
		isTicking = false;
		savedPlaybackRate;
		lastRealUserEvent = 0;
		isInitialized = false;
		...
		*/
		...
		videoMetadata = /* @__PURE__ */ new WeakMap();
	...
	//#endregion
	//#region src/platform/extension-entry.ts
	var api = globalThis.browser ?? globalThis.chrome ?? void 0;
	async function boot() {
		...
		const g = globalThis;
		g.GM_getValue = (key, fallback) => key in cache ? cache[key] : fallback;
		...
		try {
			api.storage.onChanged.addListener((changes, area) => {
				...
				for (const key of Object.keys(changes)) {
					...
					if (newValue === void 0) delete cache[key];

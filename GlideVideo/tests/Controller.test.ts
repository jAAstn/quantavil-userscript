...
vi.mock('../src/ui/styles/css', () => ({
    injectStyles: vi.fn(),
    applyTheme: vi.fn(),
    MVC_THEMES: ['contrast', 'frame', 'ember', 'abyss', 'volt'],
    MVC_THEME_LABELS: { contrast: 'High Contrast', frame: 'Frame', ember: 'Ember', abyss: 'Abyss', volt: 'Volt' },
}));
...
describe('Controller', () => {
    ...
    it('should clear all active timers in store on destroy()', () => {
        const controller = new Controller();
        controller.init();

        controller.store.timers.toast = setTimeout(() => {}, 1000);
        controller.store.timers.hide = setTimeout(() => {}, 2000);
        controller.store.timers.collapse = setTimeout(() => {}, 3000);

        expect(Object.keys(controller.store.timers).length).toBe(3);

        controller.destroy();

        expect(Object.keys(controller.store.timers).length).toBe(0);
    });
});

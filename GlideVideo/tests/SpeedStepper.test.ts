...
describe("SpeedStepper Component", () => {
	...
	it("should provide accessible button semantics and keyboard interaction on speed value", () => {
		const valEl = stepper.dom.querySelector(".mvc-stepper-pill-val") as HTMLElement;
		expect(valEl.getAttribute("role")).toBe("button");
		expect(valEl.getAttribute("tabindex")).toBe("0");
		expect(valEl.getAttribute("aria-label")).toBeTruthy();

		const playPauseSpy = vi.fn();
		eventBus.on("video:play-pause-requested", playPauseSpy);

		valEl.dispatchEvent(new KeyboardEvent("keydown", { key: "Enter" }));

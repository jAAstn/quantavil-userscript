...
describe('StateStore', () => {
    ...
    it('should load saved settings from localStorage', () => {
        ...
        mockStorage['mvc_theme'] = JSON.stringify('contrast');
    ...
    describe('getVideoId and URL cleaning', () => {
        ...
        it('should strip volatile CDN query parameters in cleanUrl', () => {
            const store = new StateStore(eventBus);
            const rawUrl = 'https://cdn.example.com/video.mp4?v-acctoken=sec123&rnd=98765&expires=1726000&token=auth123&keep=true';
            const v = { currentSrc: rawUrl, src: '', id: '' } as any;
            const videoId = (store as any).getVideoId(v);
            expect(videoId).toBe('https://cdn.example.com/video.mp4?keep=true');
        });

        it('should clear domain speed and all video positions using clear methods', () => {
            const store = new StateStore(eventBus);
            (global as any).window.location.hostname = 'example.com';
            mockStorage['mvc_lastRate_example.com'] = '2.5';
            mockStorage['mvc_lastRate'] = '2.5';
            mockStorage['mvc_positions'] = JSON.stringify({ 'https://example.com/v': 50 });
            (store as any).positions = { 'https://example.com/v': 50 };

            store.clearDomainSpeed();
            store.clearAllVideoPositions();

            expect(mockStorage['mvc_lastRate_example.com']).toBeUndefined();
            expect(mockStorage['mvc_lastRate']).toBeUndefined();
            expect(mockStorage['mvc_positions']).toBeUndefined();
            expect((store as any).positions).toEqual({});
        });
    });
});

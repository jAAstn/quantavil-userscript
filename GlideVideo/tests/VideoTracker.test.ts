...
describe('VideoTracker', () => {
    ...
    it('should traverse Document and ShadowRoot correctly in observeShadowRoots', () => {
        const spySetup = vi.spyOn(tracker, 'setupShadowRootObserver');
        ...
        tracker.observeShadowRoots(mockDoc as any);
        expect(spySetup).toHaveBeenCalledWith(mockShadowRoot as any);
    });

    it('should not fast-path return in evaluateActive if active video visibility is hidden', () => {
        const video = {
            nodeType: 1,
            tagName: 'VIDEO',
            isConnected: true,
            ...
            ended: false,
            readyState: 4,
            getBoundingClientRect: () => ({ top: 10, bottom: 500, height: 490, width: 640 }),
            closest: () => null
        } as any;

        store.visibleVideos.set(video, true);
        store.setActiveVideo(video);

        // Mock computed style to return visibility hidden
        (global as any).getComputedStyle = vi.fn().mockReturnValue({ visibility: 'hidden' });

        tracker.evaluateActive();

        // Since visibility is hidden, fast-path should NOT return early, and candidate loop also rejects it
        expect(store.activeVideo).toBeNull();
    });

    it('should trigger debouncedEvaluate on attribute mutations affecting videos', () => {
        const debouncedSpy = vi.spyOn(tracker, 'debouncedEvaluate');
        const video = {
            nodeType: 1,
            tagName: 'VIDEO',
            closest: () => null

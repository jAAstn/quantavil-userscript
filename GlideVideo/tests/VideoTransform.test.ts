...
describe('VideoTransform', () => {
    ...
    describe('Playback Speed Persistence across Videos', () => {
        beforeEach(() => {
            store.settings.rememberPlayback = true;
            store.settings.lastRate = 1.75;
        });

        it('should persist rememberedRate to new video elements on active video change even if site initialized rate to 1.0', () => {
            const newVid = {
                isConnected: true,
                playbackRate: 1.0, // site set initial rate to 1.0
                paused: true,
                style: {},
                dataset: {},
                addEventListener: vi.fn(),
            ...
            // Site ratechange fires BEFORE active video changed
            store.updateVideoMetadata(newVid, {});
            transform.handleEvent({ type: 'ratechange' } as any);

            // Now active video changes to newVid
            store.setActiveVideo(newVid);

            expect(newVid.playbackRate).toBe(1.75);
            expect(store.getVideoMetadata(newVid).lastRate).toBe(1.75);
            expect(store.settings.lastRate).toBe(1.75);
        });

        it('should not reset playback rate when video ends', () => {
            mockVideo.playbackRate = 1.75;
            store.updateVideoMetadata(mockVideo, { lastRate: 1.75 });
        ...
        it('should re-apply rememberedRate when play event fires if site set 1.0 while paused', () => {
            mockVideo.playbackRate = 1.0;
            mockVideo.paused = true;

            // Site changes rate while paused
            transform.handleEvent({ type: 'ratechange' } as any);
            expect(mockVideo.playbackRate).toBe(1.0); // Not forced while paused
        ...
        it('should reset transform and restore position on source swap on same video element', () => {
            mockVideo.src = 'https://example.com/video1.mp4';
            store.setActiveVideo(mockVideo);

            // Mutate transform on video1
            store.updateVideoMetadata(mockVideo, {
                transform: { ratio: '16:9', zoom: 1.5, rot: 90 },
                lastSrc: 'https://example.com/video1.mp4'
            });
            store.settings.transform = { ratio: '16:9', zoom: 1.5, rot: 90 };

            // Change src on the same video element and fire loadstart
            mockVideo.src = 'https://example.com/video2.mp4';
            mockVideo.currentSrc = 'https://example.com/video2.mp4';
            transform.handleEvent({ type: 'loadstart' } as any);

            // Transform should be reset to default
            const meta = store.getVideoMetadata(mockVideo);
            expect(meta.transform).toEqual({ ratio: 'fit', zoom: 1, rot: 0 });
            expect(store.settings.transform).toEqual({ ratio: 'fit', zoom: 1, rot: 0 });
        });

        it('should hide lockShield and reset brightnessOverlay when activeVideo becomes null', () => {
            const mockLockShield = { style: { display: 'block' } };
            const mockBrightnessOverlay = { style: { opacity: '0.5' } };
            (ui as any).lockShield = mockLockShield;
            (ui as any).brightnessOverlay = mockBrightnessOverlay;
            store.isLocked = true;

            // Set activeVideo to null
            transform.onActiveVideoChanged(null);

            expect(mockLockShield.style.display).toBe('none');
            expect(mockBrightnessOverlay.style.opacity).toBe('0');
